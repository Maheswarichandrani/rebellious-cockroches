...
export async function verifyCheckoutPayment(input: {
  ...
  // Fetch fresh stock for each product before reduction
    ...
    patches.push(
      writeClient
        ...
        .set({
          ...
          razorpaySignature,
          updatedAt:         new Date().toISOString(),
          // Link guest order to account if user logged in at payment time
          ...(currentUserId && !order.clerkUserId ? { clerkUserId: currentUserId } : {}),
        })
        .commit()
    )
  ...
  // Send confirmation emails (non-blocking failure)
  try {
    ...
    if (customerEmail) {
      await resend.emails.send({
        ...
        html:    await render(OrderConfirmation({ order: { ...order, paymentId: razorpayPaymentId } as any })),
      })
    }
  } catch (err) {
